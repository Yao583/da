...
doc = """
Boston mechanism played through a robot agent: the participant states a complete contingent plan
up front -- what the robot applies to when all prizes are available, and what it applies to after
being unpaired from each successive prize. One Prolific participant replaces a lab participant
per round; other choices/rankings are loaded from lab session data.
"""



class C(BaseConstants):
    NAME_IN_URL = 'agent_boston'
...
# --- Quiz 4: allocation process ---
    ...
    quiz27 = models.StringField(
        ...
        choices=[
            ['No', 'No, once you are unpaired from a prize such prize becomes unavailable to you. If you are unpaired from a prize, you can only be paired to other prizes that nobody else is paired with.'],
            ['Yes', 'Yes'],
        ],
        widget=widgets.RadioSelect,
    )
...
def _run_boston_allocation(priority_rank, rankings_by_pid):
    ...
    Round k: every still-unpaired applicant applies to their k-th ranked prize. Each prize
    keeps the highest-priority applicants up to its remaining capacity and rejects the rest.
    Pairings are permanent -- a paired applicant never applies again and their seat is gone.
    Repeats until a round produces no applications.
    ...
    while True:
        ...
        for prize_id, applicants in applicants_by_prize.items():
            ...
            winners = applicants[:remaining[prize_id]]           # the rest are rejected
            for pid in winners:
...
def _random_fill_unmatched(assigned, remaining, priority_order):
    """Part two: applicants unmatched after part one randomly receive one of the remaining seats."""
...
# --- Boston allocation logic ---
def run_boston_matching(player: Player):
    ...
    # Part one: Boston. Part two: unmatched applicants randomly take a remaining seat.
    assigned, remaining = _run_boston_allocation(priority_rank, rankings_by_pid)
...
class Decision(Page):
    ...
    def vars_for_template(player: Player):
        ...
        available_prizes = {
            prize_id for prize_id, seats_left in _build_capacity_map().items() if seats_left > 0
        }
