...
doc = """
Live sequential choice with Prolific integration: when it is the participant's turn they
are shown which prizes are still available and pick one. One Prolific participant
replaces a lab participant per round; other choices/rankings are loaded from lab session
data.
"""
...
class C(BaseConstants):
    ...
    ALIGNED = False
    ENVELOPE = False
    # Decision.html guards its Available/Taken status row with {{ if C.LIVE }}; this
    # treatment is defined by showing that availability, so it must stay True.
    LIVE = True
...
# --- Quiz 4: allocation process ---
    quiz22 = models.BooleanField(
        label="The first part of the allocation process proceeds in rounds. The ranking of the participant with the 4th prize priority is considered in round 4.",
        choices=[[True, 'True'], [False, 'False']],
        widget=widgets.RadioSelect,
    )
    quiz23 = models.BooleanField(
        label="When a participant's ranking is considered, the allocation process first determines the set of available prizes. It then assigns the participant the top prize according to the ranking they submitted among this set of available prizes.",
        choices=[[True, 'True'], [False, 'False']],
        widget=widgets.RadioSelect,
    )
    quiz24 = models.StringField(
        label="In the first part of the allocation process...",
        choices=[
            ['May be unmatched', 'A participant may be unmatched because they did not rank prizes among the ones that are available when it is their round.'],
...
# --- Quiz 6: how you submit your ranking ---
    quiz28 = models.StringField(
        label="When it is your turn:",
        choices=[
            ['Interface shows you availability', 'The interface will inform you that it is your turn and show you the prizes that are still available.'],
            ['Interface hides availability', 'The interface will inform you that it is your turn but you will not know which prizes are still available.'],
            ['No notice of your turn', 'The interface will not inform you when it is your turn.'],
        ],
        widget=widgets.RadioSelect,
    )
    quiz29 = models.StringField(
        label="If you select an available prize:",
        choices=[
            ['Matched', 'You will be matched to the prize you selected.'],
            ['Unmatched', 'You will remain unmatched.'],
        ],
        widget=widgets.RadioSelect,
    )
...
def creating_session(subsession: Subsession):
    ...
    # 3. In EVERY round (1 through 5), deal one card from the deck to the current player
    for p in subsession.get_players():
...
def _run_serial_dictatorship(priority_order, rankings_by_pid):
    """Phase 1: in priority order, each player takes a seat of their first still-available choice."""
    remaining = _build_capacity_map()
    assigned = {}
    for pid in priority_order:
        choice = None
        for prize_id in _ranking_to_prize_ids(rankings_by_pid.get(pid, '')):
            if remaining.get(prize_id, 0) > 0:
                choice = prize_id
...
def _random_fill_unmatched(assigned, remaining, priority_order):
    """Phase 2: unmatched players randomly receive one of the remaining seats."""
...
def run_sequential_rsd(player: Player):
    ...
    priority_map = {pid: rank for rank, pid in enumerate(priorities, start=1)}
    if replaced_id not in priority_map:
        raise RuntimeError(f"replaced_id={replaced_id} is missing from priorities {priorities}")
    
    # Get the lab players ordered 1st to 8th
    lab_pids_ordered = sorted(range(1, C.NUM_LAB_PLAYERS + 1), key=lambda pid: priority_map[pid])
    ...
    for pid in range(1, C.NUM_LAB_PLAYERS + 1):
        if pid == replaced_id:
            all_rankings[pid] = (player.pref_ranking or '').replace('-', '').strip().upper()
        else:
            all_rankings[pid] = lab_rankings.get(pid, '').replace('-', '')

    # Phase 1: Rank-based Allocation
    assigned, remaining = _run_serial_dictatorship(lab_pids_ordered, all_rankings)

    # Check if online player got a prize
    online_matched = assigned[replaced_id] is not None

    if online_matched:
        # Phase 2: Random Allocation for Unmatched
        _random_fill_unmatched(assigned, remaining, lab_pids_ordered)
        ...
        new_priority_pids = lab_pids_ordered.copy()
        new_priority_pids.insert(insert_index, 'online')
        new_priority_pids = new_priority_pids[:C.NUM_LAB_PLAYERS]

        # Build new rankings dict
        new_all_rankings = {}
        for pid in new_priority_pids:
            if pid == 'online':
                new_all_rankings['online'] = (player.pref_ranking or '').replace('-', '').strip().upper()
            else:
                new_all_rankings[pid] = lab_rankings.get(pid, '').replace('-', '')

        # Phase 1 + Phase 2 for new shifted group
        assigned2, remaining2 = _run_serial_dictatorship(new_priority_pids, new_all_rankings)
    ...
    my_priority = priority_map[replaced_id]
...
class InstructionsQuiz(Page):
    ...
    form_fields = ['quiz1', 'quiz2', 'quiz3', 'quiz4', 'quiz5', 'quiz6', 'quiz7', 'quiz8', 'quiz9',
        ...
        'quiz28', 'quiz29', 'quiz1_attempts', 'quiz2_attempts', 'quiz3_attempts', 'quiz4_attempts',
        'quiz5_attempts', 'quiz6_attempts', 'failed_quiz']
...
class Decision(Page):
    ...
    def vars_for_template(player: Player):
        ...
        # --- NEW LOGIC: Calculate which prizes are available before my turn ---
        lab_rankings = lab_round['rankings']

        remaining_seats = _build_capacity_map()
        priority_order = sorted(range(1, C.NUM_LAB_PLAYERS + 1), key=lambda pid: priority_map[pid])

        # Simulate choices for players who go BEFORE the online player
        for pid in priority_order:
            if priority_map[pid] < my_priority:
                ...
                break  # Stop when we reach the online player's priority

        available_prizes = {
            prize_id for prize_id, seats_left in remaining_seats.items() if seats_left > 0
        }
        player.available_prizes_at_turn = json.dumps(sorted(available_prizes))
        ...
        for prize_idx, value in enumerate(valuations, start=1):
            prize_rows.append(dict(
                ...
                value=value,
                status='Available' if prize_idx in available_prizes else 'Taken',
            ))
