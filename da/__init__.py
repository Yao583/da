...
doc = """
Deferred Acceptance with Prolific integration: the participant submits a complete ranking of
the prizes up front, and the allocation process runs server-side on that ranking. One Prolific
participant replaces a lab participant per round; other choices/rankings are loaded from lab
session data.
"""
...
class C(BaseConstants):
    ...
    ALIGNED = False
    ENVELOPE = False
    # Availability is deliberately hidden in this treatment: the participant submits a
    # ranking without knowing which prizes still have an open seat.
    LIVE = False
...
class Decision(Page):
    ...
    @staticmethod
    def js_vars(player: Player):
        # Never ship availability to the browser here: the participant submits a ranking
        # without learning which prizes still have an open seat.
